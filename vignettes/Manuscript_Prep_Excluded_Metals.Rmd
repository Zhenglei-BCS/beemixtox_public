...
N_CAS_Site <- Res %>% group_by(LOQ.type,Exclusion.CAS) %>% summarise(N.Site=length(unique(monitoringSiteIdentifier)),N.Sample=length(monitoringSiteIdentifier)) %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_None","Excluded_Metals","Current_Use")))
...
if(interactive()){
  ggsave("inst/manuscript/Figure1-Chronic.png",width = 15,height=8,dpi=300)
}
```
...
```{r}
N_CAS_Site_tmp <- Res %>% group_by(LOQ.type,Exclusion.CAS) %>% summarise(N.Site=length(unique(monitoringSiteIdentifier)),N.Sample=length(monitoringSiteIdentifier)) %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_None","Excluded_Metals","Current_Use")))
...
if(interactive()){
  write.csv(table_chronic,file="inst/manuscript/table_chronic.csv")
}
...
if(interactive()){
  write.csv(table_acute,file="inst/manuscript/table_acute.csv")
}
```
...
tmp$Exclusion.CAS <- factor(tmp$Exclusion.CAS,levels=(c("Excluded_None","Excluded_Metals","Current_Use")))
...
N_CAS_Site$Exclusion.CAS <- factor(N_CAS_Site$Exclusion.CAS,levels=(c("Excluded_None","Excluded_Metals","Current_Use")))
...
if(interactive()){
  ggsave("inst/manuscript/Figure2-Acute.png",width = 12,height=8,dpi=300)
}
...
if(interactive()){
  ggsave("inst/manuscript/Figure2-Acute-logxy.png",width = 12,height=8,dpi=300)
}
...
if(interactive()){
  ggsave("inst/manuscript/Figure2-Acute-free_x.png",width = 12,height=8,dpi=300)
}
...
if(interactive()){
  ggsave("inst/manuscript/Figure2-Chronic.png",width = 12,height=8,dpi=300)
}


ggplot(tmp,aes(x=N,y=HI_Chronic))+geom_point(pch=".")+geom_hline(yintercept = 1,col="blue")+facet_grid(LOQ.type~Exclusion.CAS+Ntype)+ annotate("rect", xmin = 0, xmax = Inf, ymin = 0, ymax = 1,fill="yellow",alpha=0.2)+geom_quantile(method = "rqss", quantiles = c(0.1, 0.5, 0.95), colour = "red")+scale_y_log10()+geom_text(data=sumtmp,aes(x=x,y=y,label=paste0("II: ",formatC(Perc,digits = 1,format="f"),"%")))+geom_text(data=N_CAS_Site,aes(x=x,y=y,label=paste0("N.Site = ",N.Site,"\nN.CAS = ",N.CAS)))+scale_x_log10()
if(interactive()){
  ggsave("inst/manuscript/Figure2-Chronic-logxy.png",width = 12,height=8,dpi=300)
}
N_CAS_Site_1<- left_join(N_CAS_Site,sumtmp[,-c(1,2)])
...
if(interactive()){
  ggsave("inst/manuscript/Figure2-Chronic_free_x.png",width = 12,height=8,dpi=300)
}
...
```{r}
driverdata <- getDriverData(Data.AggBySiteID.2,chemclass,LOQ.type = c("LOQ.T0", "LOQ.T1"), Exclusion.CAS = c("Excluded_Metals", "Current_Use"), Data.SSD.1,threshold = 1)
```
...
driverdata1 <- driverdata1 %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_Metals","Current_Use")))

nsample <- driverdata %>% group_by(LOQ.type,Exclusion.CAS,test)%>% nest()%>%mutate(sy=map(data,function(df) length(unique(df$Site.Year))),hmax=map(data,function(df) max(df$HI)))%>% select(-data) %>% unnest(cols=c(sy,hmax)) %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_Metals","Current_Use")))
...
if(interactive()){
  ggsave("inst/manuscript/Figure3-Chronic.png",width = 12,height=8,dpi=300)
}
...
```{r}
write.csv(table13,file="inst/manuscript/table13.csv")
```
...
```{r}
driverdata2 <- rbind(driverdata1 %>% filter(Exclusion.CAS=="Excluded_Metals" & test=="Chronic"),currentdat1)
driverdata2 <- driverdata2 %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_Metals","Current_Use")))%>%filter(HI>0.1)


nsample2 <- driverdata2 %>% group_by(LOQ.type,Exclusion.CAS,test)%>% nest()%>%mutate(sy=map(data,function(df) length(unique(df$Site.Year))),hmax=map(data,function(df) max(df$HI)))%>% select(-data) %>% unnest(cols=c(sy,hmax)) %>% mutate(Exclusion.CAS=factor(Exclusion.CAS,levels=c("Excluded_Metals","Current_Use")))
```
...
p1
if(interactive()){
ggsave("inst/manuscript/Figure3-Chronic-boxplot-sqrt.png",width = 12,height=8,dpi=300)
}
...
if(interactive()){
ggsave("inst/manuscript/Figure3-Chronic-boxplot.png",width = 12,height=8,dpi=300)
}
```
...
for (LOQ in c("LOQ.T0","LOQ.T1")) {
 for(Exclusion in c("Excluded_Metals","Current_Use")){
...
if(interactive()){
  ggsave("inst/manuscript/Figure3-Chronic-annotated.png",width = 12,height=8,dpi=300)
}
```
...
# https://www.blopig.com/blog/2019/08/combining-inset-plots-with-facets-using-ggplot2/
if(interactive()){
  ggsave("inst/manuscript/Figure3-Chronic-annotated-sqrt.png",width = 12,height=8,dpi=300)
}
```
...
if(interactive()){
ggsave("inst/manuscript/Figure3-Chronic-boxplot-Chronic-Site.Year-sqrt.png",width = 12,height=8,dpi=300)
}
...
if(interactive()){
ggsave("inst/manuscript/Figure3-Chronic-boxplot-Chronic-Site.Year.png",width = 12,height=8,dpi=300)
}
```
...
# https://www.blopig.com/blog/2019/08/combining-inset-plots-with-facets-using-ggplot2/
if(interactive()){
  ggsave("inst/manuscript/Figure3-Chronic-annotated-site-year.png",width = 12,height=8,dpi=300)
}
```
...
# https://www.blopig.com/blog/2019/08/combining-inset-plots-with-facets-using-ggplot2/
if(interactive()){
  ggsave("inst/manuscript/Figure3-Chronic-annotated-sqrt-site-year.png",width = 12,height=8,dpi=300)
}
```
...
```{r}
write.csv(sumCAS,file="inst/manuscript/table_fig4a.csv")
```
...
if(interactive()){
  ggsave("inst/manuscript/Figure4a-Chronic.png",width = 12,height=8,dpi=300)
}
```
...
#ggplot(sumCAS%>%filter(Exclusion.CAS=="Excluded_Metals" & test=="Acute"&LOQ.type=="LOQ.T0"),aes(x=reorder(Substance,freq),y=freq,fill=Chem.Group))+geom_bar(stat="identity")+ coord_flip()+facet_wrap(LOQ.type~test,scale="free",drop=TRUE)
if(interactive()){
  ggsave("inst/manuscript/Figure4a-Chronic-Current_Use.png",width = 12,height=8,dpi=300)
}
```
...
```{r}
#driverdata <- getDriverData(Data.AggBySiteID.2,chemclass,LOQ.type = c("LOQ.T0", "LOQ.T1"), Exclusion.CAS = c("Excluded_Metals", "Current_Use"), Data.SSD.1,include0 = T)
HQdata <- getHQData_Chronic(Data.AggBySiteID.2,chemclass=chemclass,Data.SSD.1=Data.SSD.1,LOQ.type= c("LOQ.T0", "LOQ.T1"),driverdata,Exclusion.CAS="Excluded_Metals")
```
...
```{r}
write.csv(sumCAS,file="inst/manuscript/table_fig4b_Excluded_Metals_PAH.csv")
```
...
if(interactive()){
  ggsave("inst/manuscript/Figure4b-Chronic.png",width = 12,height=9,dpi=300)
}
...
```{r}
print("Excluded_Metals finished!")
```
