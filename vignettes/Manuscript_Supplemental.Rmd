...
ggplot(some.eu.maps, aes(x = long, y = lat)) +
  ...
  theme_void()+
  geom_point(data=stations1%>%filter(LOQ.type=="LOQ.T0" & Exclusion.CAS=="Excluded_None"),aes(x = lon, y = lat,col=log(mean_HI_Acute)))+geom_text(aes(label = region), data = region.lab.data,  size = 3, hjust = 0.5)+scale_color_viridis_c(direction=-1,option = "inferno")+guides(fill=FALSE,alpha=FALSE)
```
...
```{r}
ggplot(data=stations1%>%filter(Exclusion.CAS != "Excluded_None"& Exclusion.CAS != "Refined_Metals_PAH"),aes(x = lon, y = lat))+geom_point(aes(col=(mean_HI_Chronic)))+
