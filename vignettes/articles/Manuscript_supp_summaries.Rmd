...
b <- Data.AggBySiteID %>% group_by(phenomenonTimeReferenceYear) %>% summarise(nsample=n()) %>% arrange(phenomenonTimeReferenceYear)%>%mutate(Data="Before Data Selection")
yeardata <- rbind(a,b)%>%mutate(Data=factor(Data,levels=c("Before Data Selection","After Data Curation")))
...
if(interactive()) ggsave("~/Projects/beemixtox/inst/manuscript_v3/supp_fig2.png",width=5,height=3)
```
...
b <-   tmp%>%group_by(countryCode) %>% summarise(nsample=n()) %>% arrange(countryCode)%>%mutate(Data="Before Data Selection")
countrydata <- rbind(a,b)%>%mutate(Data=factor(Data,levels=c("Before Data Selection","After Data Curation")))
...
if(interactive()) ggsave("~/Projects/beemixtox/inst/manuscript_v3/supp_fig1.png",width=5,height=3)
```
...
parent <- c("",rep("All",nrow(tmp)),parent)
labels <- c(paste0("All (",sum(tmp$Ndetected),"\n",nrow(Data.AggBySiteID.2),")"),paste0(tmp$class1,"\n",tmp$Ndetected,"\n",tmp$Nmeasured),labels)
